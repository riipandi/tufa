...
import dev.altaris.tufa.ViewMode;
...
import dev.altaris.tufa.importers.TufaImporter;
import dev.altaris.tufa.importers.DatabaseImporter;
import dev.altaris.tufa.importers.DatabaseImporterEntryException;
import dev.altaris.tufa.importers.DatabaseImporterException;
import dev.altaris.tufa.services.NotificationService;
import dev.altaris.tufa.ui.models.ImportEntry;
import dev.altaris.tufa.ui.preferences.SwitchPreference;
import dev.altaris.tufa.util.UUIDMap;
import dev.altaris.tufa.vault.VaultBackupManager;
...
public class PreferencesFragment extends PreferenceFragmentCompat {
    ...
    private TufaImporter.State _importerState;
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        TufaApplication app = (TufaApplication) getActivity().getApplication();
    ...
    private void importDatabase(DatabaseImporter importer, DatabaseImporter.FileReader reader) {
        try {
            ...
            if (state.isEncrypted()) {
                // temporary special case for encrypted Tufa vaults
                if (state instanceof TufaImporter.EncryptedState) {
                    _importerState = state;

                    Intent intent = new Intent(getActivity(), AuthActivity.class);
                    intent.putExtra("slots", ((TufaImporter.EncryptedState) state).getSlots());
    ...
    private void onImportDecryptResult(int resultCode, Intent data) {
        ...
        try {
            state = ((TufaImporter.EncryptedState) _importerState).decrypt(creds);
